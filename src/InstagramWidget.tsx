...
export default function InstagramWidget() {
  ...
  useEffect(() => {
    const savedStats = localStorage.getItem('ig_stats');
    const savedAvatar = localStorage.getItem('ig_avatar');
  ...
  const setAvatarUrl = () => {
    ...
    img.onload = () => {
      setAvatar(url);
      localStorage.setItem('ig_avatar', url);
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      ...
      setAvatar(dataUrl);
      localStorage.setItem('ig_avatar', dataUrl);
    };
    reader.readAsDataURL(file);
  };

  const handleStatEdit = (key: 'posts' | 'followers' | 'following', value: string) => {
    const newStats = { ...stats, [key]: value };
    setStats(newStats);
    localStorage.setItem('ig_stats', JSON.stringify(newStats));
  };
  ...
  return (
    <>
      <style>{`
        ...
        .stat-num { font-size: 17px; font-weight: 700; display: block; border-radius: 4px; padding: 1px 4px; min-width: 30px; }
        .stat-num.editable { cursor: pointer; transition: background 0.2s; }
        .stat-num.editable:hover { background: #2a2a2a; }
        .stat-num.editing { outline: 1px solid #e1306c; background: #2a2a2a; }
      ...
      <div className="ig-card">
        ...
        <div className="ig-stats">
          <div>
            <span 
              className={`stat-num${adminMode ? ' editable' : ''}${editingKey === 'posts' ? ' editing' : ''}`}
              contentEditable={adminMode}
              onBlur={(e) => {
                const value = (e.currentTarget.textContent || '').trim() || stats.posts;
                handleStatEdit('posts', value);
                setEditingKey(null);
              }}
              onKeyDown={(e) => {
                if (e.key === 'Enter') {
          ...
          <div>
            <span 
              className={`stat-num${adminMode ? ' editable' : ''}${editingKey === 'followers' ? ' editing' : ''}`}
              contentEditable={adminMode}
              onBlur={(e) => {
                const value = (e.currentTarget.textContent || '').trim() || stats.followers;
                handleStatEdit('followers', value);
                setEditingKey(null);
              }}
              onKeyDown={(e) => {
                if (e.key === 'Enter') {
              ...
              onFocus={() => setEditingKey('followers')}
            >
              {stats.followers}
            </span>
          ...
          <div>
            <span 
              className={`stat-num${adminMode ? ' editable' : ''}${editingKey === 'following' ? ' editing' : ''}`}
              contentEditable={adminMode}
              onBlur={(e) => {
                const value = (e.currentTarget.textContent || '').trim() || stats.following;
                handleStatEdit('following', value);
                setEditingKey(null);
              }}
              onKeyDown={(e) => {
                if (e.key === 'Enter') {
              ...
              onFocus={() => setEditingKey('following')}
            >
              {stats.following}
            </span>
